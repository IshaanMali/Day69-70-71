...
def show_post(post_id):
    ...
    users_result = db.session.execute(db.select(User))
    users = result.scalars()
    if form.validate_on_submit():
        try:
            id = current_user.id
        except AttributeError:
            flash("You need to log-in to comment on a post.")
            return redirect(url_for("login"))
        else:
            with app.app_context():
                comment_data = form.comment.data
                comment = Comment(text=comment_data)
    ...
    return render_template("post.html", post=requested_post,current_user=current_user,form=form,comments=comments,users=users)
